...
class Session:
    ...
    def __init__(self, filename, load):
        ...
        generator.settings.token_repetition_penalty_max = saved.get(
        ...
        generator.settings.token_repetition_penalty_decay = saved.get(
            "token_repetition_penalty_decay", 512
        )
    ...
    def api_populate(self):
        ...
        model_str = (
            os.path.splitext(os.path.basename(model.config.model_path))[0] + "\n"
        )
    ...
    def api_set_gen_settings(self, data):
        ...
        generator.settings.token_repetition_penalty_max = data[
            "token_repetition_penalty_max"
        ]
        generator.settings.token_repetition_penalty_sustain = data[
            "token_repetition_penalty_sustain"
        ]
        generator.settings.token_repetition_penalty_decay = data[
    ...
    def set_context_window(self):
        ...
        max_context_tokens = (
            model.config.max_seq_len - self.chunk_size - generator.settings.beam_length
        )
    ...
    def respond(
        self, author, stop_conditions, total_tokens, res_line="", num_res_tokens=0
    ):
        ...
        for i in range(self.max_response_tokens):
            ...
            if generator.sequence_actual is not None:
                if (
                    generator.sequence_actual.shape[-1]
                    + self.chunk_size
                    + generator.settings.beam_length
                    + 1
            ...
            for stop_tokens, stop_string in stop_conditions:
                if res_line.lower().endswith(stop_string.lower()):
                    generator.gen_rewind(
                        ...
                        - (
                            1
                            if stop_tokens[0, 0].item() == tokenizer.newline_token_id
                            else 0
                        )
                    )
