...
class Perplexity:
    ...
    def test(self, chunk_limit=sys.maxsize, lora=None, tag="", ppl_token=False):
        ...
        print(
            f" -- Testing {min(len(self.dataset_chunks), chunk_limit)} chunks", end=""
        )
        sys.stdout.flush()
        ...
        for chunk in self.dataset_chunks:
            ...
            if ppl_token:
                ...
                for i in range(input_ids.shape[-1]):
                    logits_t = self._next_logits(
                        input_ids[:, i : i + 1], lora, last_id_only=False
                    )
...
def post_parse(args):
    ...
    print(
        ...
        + (
            f" -> {args.perplexity_chunk_truncate}"
            if args.perplexity_chunk_truncate is not None
            else ""
        )
    )
