...
if sys.version_info < min_version:
    print("")
    print(
        f" ## Warning: this project requires Python {min_version[0]}.{min_version[1]} or higher."
    )
    print("")
...
class Ex4bitLinear:
    def __init__(self, config, in_features, out_features, has_bias, tensors, key):
        ...
        self.g_idx = (
            tensors[key + ".g_idx"].cpu() if key + ".g_idx" in tensors else None
        )
        ...
        self.q4 = cuda_ext.ext_make_q4(
            self.qweight, self.qzeros, self.scales, self.g_idx, self.device_index
        )
    ...
    def fused(self, x, buffer, post_attention_layernorm, lora):
        ...
        if temp_size > 0:
            lora_temp = torch.empty(
                (1, temp_size), dtype=torch.float16, device=x.device
            )
        else:
...
class ExLlamaRMSNorm:
    ...
    def forward(self, hidden_states, buffer):
        hidden_states = cuda_ext.ext_rms_norm(
...
class ExLlamaAttention:
    ...
    def fused(self, hidden_states, cache, buffer, input_layernorm, lora):
        ...
        if temp_size > 0:
            lora_temp = torch.empty(
                (1, temp_size), dtype=torch.float16, device=hidden_states.device
            )
        else:
            lora_temp = cuda_ext.none_tensor
        ...
        if self.config.use_flash_attn_2 and (past_len == 0 or q_len == 1):
            ...
            attn_output = flash_attn_func(
                query_states, key_states, value_states, causal=(past_len == 0)
            )
        ...
        cuda_ext.exllama_ext.q4_attn_2(
            hidden_states, attn_output, self.o_proj.q4, o_a, o_b, lora_temp
        )
        # return hidden_states
    ...
    def forward(self, hidden_states, cache, buffer, lora):
        ...
        value_states = (
            ...
            .view(bsz, q_len, self.config.num_key_value_heads, self.config.head_dim)
            .transpose(1, 2)
        )
        ...
        new_values = (
            cache.value_states[self.index].narrow(2, past_len, q_len).narrow(0, 0, bsz)
        )
        ...
        # -- HF Transformers regular attention, faster on shorter sequences, same VRAM usage
            ...
            value_states = self.repeat_kv(
                value_states, self.config.num_key_value_groups
            )
...
class ExLlamaDecoderLayer:
    def __init__(self, config, tensors, key, index, sin, cos):
        ...
        self.self_attn = ExLlamaAttention(
            self.config, tensors, key + ".self_attn", sin, cos, self.index
        )
    ...
    def forward(self, hidden_states, cache, buffer, lora):
        ...
        if self.config.fused_attn and _rows(hidden_states) == 1:
            self.self_attn.fused(
                hidden_states, cache, buffer, self.input_layernorm, lora
            )
        ...
        if (
            self.config.fused_mlp_thd > 0
            and _rows(hidden_states) <= self.config.fused_mlp_thd
        ):
...
class ExLlamaDeviceMap:
    ...
    def get_all_devs(self):
        return sorted(
            list(set(self.layers + [self.lm_head, self.norm, self.embed_tokens]))
        )
...
class ExLlamaBuffer:
    ...
    def to(self, device):
        ...
        new.attn_mask = (
            ...
            else _move_tensor(self.attn_mask, device, "attn_mask", self.config)
        )
        return new
...
class ExLlama:
    def __init__(self, config):
        ...
        for dev in self.config.device_map.get_layers_devs():
            ...
            temp_state = torch.zeros(
                (config.max_input_len, config.intermediate_size),
                dtype=torch.float16,
                device=dev,
            )
            temp_mlp = torch.zeros(
                (config.fused_mlp_thd * 2, config.intermediate_size),
                dtype=torch.float16,
                device=dev,
            )
            temp_zeros_float = torch.zeros((1, 65536), dtype=torch.float32, device=dev)
            temp_dq = torch.zeros(
                (1, max_dq_buffer_size), dtype=torch.float16, device=dev
            )
    ...
    def forward(
        ...
        assert input_mask is None or (
            input_mask.shape[-1] >= input_ids.shape[-1]
            and input_mask.shape[-2] == input_ids.shape[-2]
        )
        ...
        with torch.no_grad():
            ...
            if seq_len > 1 or input_mask is not None:
                ...
                attn_mask_triu = torch.triu(
                    torch.full((seq_len - 1, seq_len - 1), -65504.0)
                )
                attn_mask[
                ...
                if input_mask is not None:
                    ...
                    input_mask = _move_tensor(
                        input_mask, devs[0], "input_mask", self.config
                    )
            ...
            input_ids = _move_tensor(
                input_ids, self.config.device_map.embed_tokens, "input_ids", self.config
            )
            ...
            for i, decoder_layer in enumerate(self.layers):
                ...
                hidden_states = decoder_layer.forward(
                    hidden_states, cache, buffers[device], lora
                )
            ...
            hidden_states = _move_tensor(
                hidden_states,
                self.config.device_map.lm_head,
                "hidden_states",
                self.config,
            )
