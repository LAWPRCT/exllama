...
class ExLlamaTokenizer:
            ...
            if return_mask:
                if needs_mask:
                    mask_padding = torch.full(
                        (stacked_ids.shape[0], max_seq_len - stacked_ids.shape[1]),
                        True,
                        dtype=torch.bool,
                        device="cpu",
                    )
                    ...
                    for segment in split_text:
                        if isinstance(segment, str) and special_character in segment:
                            ...
                            for i, part in enumerate(parts):
                                ...
                                if (
                                    i < len(parts) - 1
                                ):  # add the special token id between parts, but not after the last part
    ...
    def decode(self, ids, decode_special_characters=False):
        special_ids = {
            id_: char for char, id_ in self.special_characters
        ...
        if ids.dim() > 1:
            ...
            for i in range(ids.shape[0]):
                ...
                if decode_special_characters:
                    ...
                    normal_ids.append(
                        current_normal_ids
                    )  # save the last segment of normal IDs

                    decoded_segments = [
                        self.tokenizer.Decode(segment) for segment in normal_ids
                    ]
