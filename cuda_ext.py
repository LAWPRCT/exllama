...
if windows:
    ...
    def find_msvc():
        for msvc_dir in [
            ...
            for version in versions:
                ...
                if os.path.exists(compiler_dir) and os.path.exists(
                    compiler_dir + "\\cl.exe"
                ):
                    return compiler_dir
        return None
    ...
    try:
        ...
        if cl_path:
            ...
            print(
                "Unable to find cl.exe; compilation will probably fail.",
                file=sys.stderr,
            )
...
exllama_ext = load(
    ...
    extra_ldflags=(
        ["cublas.lib"]
        + (
            [f"/LIBPATH:{os.path.join(sys.base_prefix, 'libs')}"]
    ...
    + (["-U__HIP_NO_HALF_CONVERSIONS__", "-O3"] if torch.version.hip else []),
    extra_cflags=["-O3"]
