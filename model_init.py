...
def add_args(parser):
    ...
    parser.add_argument(
        "-c", "--config", type=str, help="Model config path (config.json)"
    )
    parser.add_argument(
        "-m", "--model", type=str, help="Model weights path (.pt or .safetensors file)"
    )
    parser.add_argument(
        "-d",
        "--directory",
    ...
    parser.add_argument(
        "-l", "--length", type=int, help="Maximum sequence length", default=2048
    )
    parser.add_argument(
        "-cpe",
        "--compress_pos_emb",
    ...
    parser.add_argument(
        ...
        "--gpu_peer_fix",
        action="store_true",
        help="Prevent direct copies of data between GPUs",
    )
    ...
    parser.add_argument(
        ...
        const="default",
        metavar="METHOD",
        help="Use Flash Attention with specified input length (must have Flash Attention 2.0 installed)",
    )
    ...
    parser.add_argument(
        ...
        "--matmul_fused_remap",
        action="store_true",
        help="Fuse column remapping in Q4 matmul kernel",
    )
    parser.add_argument(
        "-nfa", "--no_fused_attn", action="store_true", help="Disable fused attention"
    )
    ...
    parser.add_argument(
        ...
        "--rmsnorm_no_half2",
        action="store_true",
        help="Don't use half2 in RMS norm kernel",
    )
    parser.add_argument(
        "-rpnh2",
        "--rope_no_half2",
        action="store_true",
        help="Don't use half2 in RoPE kernel",
    )
    parser.add_argument(
        "-mmnh2",
        "--matmul_no_half2",
        action="store_true",
        help="Don't use half2 in Q4 matmul kernel",
    )
    parser.add_argument(
        "-snh2",
        "--silu_no_half2",
        action="store_true",
        help="Don't use half2 in SiLU kernel",
    )
    parser.add_argument(
        "-nh2",
        "--no_half2",
        action="store_true",
        help="(All of the above) disable half2 in all kernela",
    )
    parser.add_argument(
        "-fh2",
        "--force_half2",
        action="store_true",
        help="Force enable half2 even if unsupported",
    )
    parser.add_argument(
        "-cs",
        "--concurrent_streams",
        action="store_true",
        help="Use concurrent CUDA streams",
    )
    ...
    parser.add_argument(
        ...
        "--affinity",
        type=str,
        help="Comma-separated list, sets processor core affinity. E.g.: -aff 0,1,2,3",
    )
...
def print_options(args, extra_options=None):
    ...
    if args.affinity:
        print_opts.append(f" --affinity: {args.affinity}")
    ...
    if args.flash_attn:
        ...
        print(
            f" -- --sdp_thd: {args.sdp_thd}"
            + (" (disabled)" if args.sdp_thd == 0 else "")
        )
    ...
    print(
        f" -- --fused_mlp_thd: {args.fused_mlp_thd}"
        + (" (disabled)" if args.fused_mlp_thd == 0 else "")
    )
    if args.matmul_fused_remap:
        print(f" -- --matmul_fused_remap")
    if args.no_fused_attn:
        print(f" -- --no_fused_attn")
    if args.rmsnorm_no_half2:
        print(f" -- --rmsnorm_no_half2")
    if args.rope_no_half2:
        print(f" -- --rope_no_half2")
    if args.matmul_no_half2:
        print(f" -- --matmul_no_half2")
    if args.silu_no_half2:
        print(f" -- --silu_no_half2")
    if args.concurrent_streams:
        print(f" -- --concurrent_streams")
...
def set_globals(args):
    if args.affinity:
        set_affinity_str(args.affinity)
