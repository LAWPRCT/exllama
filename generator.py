...
class ExLlamaGenerator:
    class Settings:
        temperature = 0.95
        top_k = (
            40  # consider the most probable top_k samples, 0 to disable top_k sampling
        )
        ...
        typical = (
            0.0  # Locally typical sampling threshold, 0.0 to disable typical sampling
        )
        ...
        token_repetition_penalty_decay = (
            128  # Gradually decrease penalty over this many tokens
        )
    ...
    def batched_sample(self, logits, temperature, top_k, top_p, min_p, typical, num=1):
        ...
        for i in range(logits.shape[0]):
            t, s = self.sample(
                logits[i, :, :], temperature, top_k, top_p, min_p, typical
            )
            samples.append(t)
            scores.append(s)
    ...
    def sample(self, logits, temperature, top_k, top_p, min_p, typical, num=1):
        ...
        sampled_ind = torch.multinomial(
            top_probs,
            top_probs.shape[-1] if num == -1 else min(num, top_probs.shape[-1]),
        )
    ...
    def gen_begin_reuse(self, in_tokens, mask=None):
        ...
        if reuse < in_tokens.shape[-1]:
            self.gen_feed_tokens(in_tokens[:, reuse:], mask=mask)
        return reuse
    ...
    def generate_simple(self, prompt, max_new_tokens=128):
        ...
        ids, mask = self.tokenizer.encode(
            prompt, return_mask=True, max_seq_len=self.model.config.max_seq_len
        )
        self.gen_begin(ids, mask=mask)

        max_new_tokens = min(
            max_new_tokens, self.model.config.max_seq_len - ids.shape[1]
        )
        ...
        text = self.tokenizer.decode(
            self.sequence[0] if self.sequence.shape[0] == 1 else self.sequence
        )
        return text
    ...
    def gen_single_token(self, constraints=None, mask=None):
        ...
        if self.sequence is not None:
            logits = self.model.forward(
                self.sequence[:, -1:], self.cache, lora=self.lora, input_mask=mask
            )
    ...
    class Beam:
        ...
        def __init__(
            self, settings, generator, first_token=None, first_prob=None, seq_pos=None
        ):
            ...
            self.probs = (
                first_prob.unsqueeze(0).unsqueeze(0) if first_prob is not None else None
            )
            self.cache = ExLlamaCache(
                self.generator.model, max_seq_len=self.settings.beam_length
            )
        ...
        def to_sequence(self):
            ...
            if new_tokens > added_tokens:
                self.generator.sequence[0, -new_tokens:] = self.sequence[
                    0, -new_tokens:
                ]
        ...
        def record_last_cache_column(self):
            self.generator.cache.copy_states(
                self.cache,
                self.generator.cache.current_seq_len - 1,
                1,
                len(self) - 1,
                1,
                0,
    ...
    def beam_search(self):
        ...
        max_beam_length = min(
            self.model.config.max_seq_len - self.settings.beam_length,
            self.settings.beam_length,
        )
        while self.beams is None or len(self.beams[0]) < max_beam_length:
            if self.beams is None:
                ...
                # self.cache.debug()
                logits = self.model.forward(
                    self.sequence[:, -1:], self.cache, lora=self.lora
                )
                ...
                while len(self.beams) < min(self.settings.beams, tokens.shape[-1]):
                    beam = ExLlamaGenerator.Beam(
                        self.settings,
                        self,
                        tokens[0, len(self.beams)],
                        probs[0, len(self.beams)],
                        c_seq_len,
                    )
                ...
                for i, beam in enumerate(self.beams):
                    ...
                    beams_.append(
                        torch.Tensor([i] * beam.sampled_tokens.shape[-1]).to(torch.int)
                    )
                ...
                for i in range(len(beams_all)):
                    ...
                    if not self.beams[beam_idx].moved:
                        ...
                        self.beams[beam_idx].probs = torch.cat(
                            (
                                self.beams[beam_idx].probs,
                                new_prob.unsqueeze(0).unsqueeze(0),
                            ),
                            dim=1,
                        )
        ...
        self.sequence_actual = torch.cat(
            (self.sequence_actual, best_token.unsqueeze(0)), dim=1
        )
